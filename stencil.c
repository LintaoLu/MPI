 ...
 // create Cartesian topology
  ...
  MPI_Cart_shift(topocomm, 1, 1, &north, &south);
    ...
    for (iter = 0; iter < niters; ++iter) {
        ...
        double *send_data = (double*) malloc(sizeof(double)*(2*bx+2*by));
        double *rec_data = (double*) malloc(sizeof(double)*(2*bx+2*by));
        int *data_position = (int*) malloc(sizeof(int)*4);
        int *data_count = (int*) malloc(sizeof(int)*4);

        pack_data(bx, by, aold, send_data);
        ...
        data_position[0] = 0;        
        data_position[1] = bx;
        data_position[2] = 2*bx;
        data_position[3] = 2*bx+by;
        
        MPI_Neighbor_alltoallv(send_data, data_count, data_position, MPI_DOUBLE, rec_data, data_count,
                      data_position, MPI_DOUBLE, topocomm);
        ...
        /* unpack data */
        unpack_data(bx, by, aold, rec_data);
               ...
               double *send_data)
{
    int i;
    for (i = 0; i < bx; ++i)
        send_data[i] = aold[ind(i + 1, 1)];     /* #1 row */
    for (i = 0; i < bx; ++i)
        send_data[i + bx] = aold[ind(i + 1, by)];    /* #(by) row */
    for (i = 0; i < by; ++i)
        send_data[i + 2 * bx] = aold[ind(bx, i + 1)];     /* #(bx) col */
    for (i = 0; i < by; ++i)
        send_data[i + 2 * bx + by] = aold[ind(1, i + 1)];      /* #1 col */
}
                 ...
                 double *rec_data)
{
    int i;
    for (i = 0; i < bx; ++i)
        aold[ind(i + 1, 0)] = rec_data[i];     /* #0 row */
    for (i = 0; i < bx; ++i)
        aold[ind(i + 1, by + 1)] = rec_data[i + bx];        /* #(by+1) row */
    for (i = 0; i < by; ++i)
        aold[ind(bx + 1, i + 1)] = rec_data[i + 2 * bx]; /* #(bx+1) col */
    for (i = 0; i < by; ++i)
        aold[ind(0, i + 1)] = rec_data[i + 2 * bx + by];      /* #0 col */
}
